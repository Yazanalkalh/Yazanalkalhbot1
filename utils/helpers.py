...
# This file contains helper functions. The get_live_time_str function
# has been upgraded with a translation dictionary for city names.
...
def get_live_time_str():
    ...
    It now translates common city names into Arabic.
    """
    try:
        tz_name = data_store.bot_data.get('ui_config', {}).get('timezone', 'Asia/Riyadh')
        user_tz = pytz.timezone(tz_name)
        # Get the English location name from the timezone string
        location_name_en = tz_name.split('/')[-1].replace('_', ' ')
    except pytz.UnknownTimeZoneError:
        user_tz = pytz.timezone('Asia/Riyadh')
        location_name_en = "Riyadh"

    # --- NEW: The Translation Dictionary ---
    translation_dict = {
        "Riyadh": "الرياض",
        "Aden": "عدن",
        "Sana'a": "صنعاء", # Common variation for Sana
        "Cairo": "القاهرة",
        "Dubai": "دبي",
        "Mecca": "مكة المكرمة"
    }
    ...
    # Use the translated Arabic name in the final string
