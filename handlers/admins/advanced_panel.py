...
# This is the final, complete, and fully functional "Engine Room Manager".
...
async def advanced_callbacks_cmd(cq: types.CallbackQuery, state: FSMContext):
    ...
    if d == "adv_view_library":
        ...
        if not content_list:
            ...
            for i, item in enumerate(content_list):
                ...
                keyboard.add(types.InlineKeyboardButton(f"🗑️ `{item.get('type')}`: {snippet}", callback_data=f"adv_delete_lib_idx_{i}"))
    ...
    if d.startswith("adv_review_"):
        ...
        keyboard.add(types.InlineKeyboardButton("✅ موافقة", callback_data=f"adv_approve_{chat_id}"),
