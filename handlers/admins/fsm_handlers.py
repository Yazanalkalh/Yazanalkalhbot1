...
async def delete_channel_msg_handler(m: types.Message, state: FSMContext):
    try:
        ...
        if 0 <= idx < len(messages):
            ...
            await m.reply(f"❌ رقم غير صالح. (1 - {len(messages)})")
...
async def date_button_label_handler(m: types.Message, state: FSMContext):
    ...
    await m.reply(f"✅ تم تحديث اسم زر التاريخ إلى: `{value}`", reply_markup=create_admin_panel()); await state.finish()
...
async def time_button_label_handler(m: types.Message, state: FSMContext):
    ...
    await m.reply(f"✅ تم تحديث اسم زر الساعة إلى: `{value}`", reply_markup=create_admin_panel()); await state.finish()
...
async def reminder_button_label_handler(m: types.Message, state: FSMContext):
    ...
    await m.reply(f"✅ تم تحديث اسم زر التذكير إلى: `{value}`", reply_markup=create_admin_panel()); await state.finish()
...
async def set_timezone_handler(m: types.Message, state: FSMContext):
    ...
    try:
        ...
        await m.reply(f"✅ تم تحديث المنطقة الزمنية إلى: `{corrected_tz_name}`", reply_markup=create_admin_panel())
    except pytz.UnknownTimeZoneError: await m.reply(f"❌ **منطقة زمنية غير صالحة:** `{tz_name_input}`\nمثال: `Asia/Riyadh`")
    await state.finish()
...
async def set_channel_id_handler(m: types.Message, state: FSMContext):
    ...
    await m.reply(f"✅ تم تحديث ID القناة إلى: `{channel_id}`", reply_markup=create_admin_panel()); await state.finish()
